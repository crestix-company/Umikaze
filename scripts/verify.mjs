...
const pagesConfig=JSON.parse(readFileSync(new URL('../wrangler.json',import.meta.url),'utf8'));
assert.equal(pagesConfig.name,'umikaze','Cloudflare Pages project must remain umikaze');
assert.equal(pagesConfig.pages_build_output_dir,'./dist','Cloudflare Pages must publish dist, not out or the repository root');

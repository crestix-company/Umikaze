...
const base=process.argv[2];if(base){for(const file of [...files,...assets,'assets/site.css','assets/layout.css','assets/motion.css','assets/site.js']){const response=await fetch(new URL(file,base));assert.equal(response.status,200,`${file}: HTTP ${response.status}`);assert(Buffer.from(await response.arrayBuffer()).equals(readFileSync(resolve(root,file))),`${file}: served bytes differ`)}}
